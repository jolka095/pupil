class Fixation(object):
    ...
    def get_fixation_id(self, dictionary):
        if 'id' in dictionary:  # v <= 1.12
            return dictionary['id']
        if 'fixation_id' in dictionary:  # v >= 1.13
            return dictionary['fixation_id']

    def __get_parameter_from_dict__(self, dictionary, key):
        return dictionary[key] if (key in dictionary) else "NA"

    def __init__(self, dictionary):
        self.id = self.get_fixation_id(dictionary)  # self.__get_parameter_from_dict__(dictionary, "id")
